...
class ArticalControllers {
  ...
  async updateArtical (ctx) {
    ...
    await model
      .updateArtical(data)
      .then(value => { ctx.body = { success: true } })
      .catch(() => { ctx.body = {success: false, error: '保存文章失败！'} })
  }
  async addArtical (ctx) {
    await model.addArtical()
      .then(value => {
        ctx.body = { success: true }
      })
      .catch(() => { ctx.body = { success: false, error: '添加文章失败！' } })
  }
  async deleteArtical (ctx) {
    const id = ctx.request.body.id
    await model.deleteArtical(id)
      .then(value => {
      ...
      .catch((err) => { ctx.body = { success: false, error: '删除文章失败！', err } })
  }
  async publishArtical (ctx) {
    let id = ctx.request.body.id
    await model.publishArtical(id).then(value => {
      if (value.affectedRows === 1) {
        ctx.body = { success: true }
      } else {
        throw new Error()
      }
    }).catch(() => { ctx.body = { success: false, error: '发布文章失败！' } })
  }
  async unpublishArtical (ctx) {
    let id = ctx.request.body.id
    await model.unpublishArtical(id).then(value => {
      if (value.affectedRows === 1) {
        ctx.body = { success: true }
      } else {
        throw new Error()
      }
    }).catch((err) => { ctx.body = { success: false, error: '隐藏文章失败！', err } })
  }
}
